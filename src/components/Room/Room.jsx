		...
		<div className="outerContainer d-flex align-items-center min-vh-100">
			<div className="container">
				<div className="row justify-content-around">
					...
					{spinner === true ? (
						<FontAwesomeIcon icon={faPencilAlt} className="spinner mx-auto my-auto col-lg-1 p-1" />
					) : null}
					<div className="col-lg-3">{button}</div>
					<div className="col timer mainHeader">
						<h3 className="text-center">Time left: {info !== "" ? counter : "updating..."}</h3>
					</div>
				...
				<div className="row justify-content-center">
					<div
						id="canvas"
						className={guessCorrect === true ? "col-11 col-lg-6 correct" : "col-11 col-lg-6"}
					>
						{canvas}
					</div>
						...
						{participants.map((p) => {
							return (
								<div className="row justify-content-around" key={"id" + p.id} id={"id" + p.id}>
									<h3 className={p.name === name ? "me text-center" : "names text-center"}>
										{String.fromCodePoint(p.avatar)}
